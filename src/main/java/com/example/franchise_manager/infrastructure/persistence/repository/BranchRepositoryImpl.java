...
public class BranchRepositoryImpl implements BranchRepository {
    ...
    @Override
    public Branch saveNew(Branch branch, Long franchiseId) {
    ...
    public Branch save(Branch branch) {
        BranchEntity entity = jpaBranchRepository.findById(branch.getId())
                .orElseThrow(() -> new IllegalArgumentException("Branch not found: " + branch.getId()));

        // Add new products (those without id)
    ...
    private Branch mapToDomain(BranchEntity entity) {
        ...
        if (entity.getProducts() != null) {
            entity.getProducts().forEach(pe ->
                    branch.addProduct(new Product(pe.getId(), pe.getName(), pe.getStock()))
            );
        }
